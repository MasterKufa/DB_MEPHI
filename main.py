...
class Main:
    ...
    def db_insert_somethings(self):
        ...
        grt.insert_one([1, "C18-702", "Infanalit", "fin monitoring"])
    ...
    def after_show_people(self, next_step):
        while True:
            if next_step == "4":
                ...
                next_step = self.show_groups_by_people()
                while next_step == "6" or next_step == "7":
                    if next_step == "6":
                        next_step = self.show_add_group()
                    elif next_step == "7":
                        next_step = self.delete_group()
                if next_step != "0" and next_step != "1" and next_step != "9":
                    print("Выбрано неверное число! Повторите ввод!")
                    next_step = "6"

            elif next_step == "7":
                next_step = self.find_person_by_last_name()
    ...
    def find_person_by_last_name(self):
        while True:
            last_name = str(input(prompts["search_by_last_name"]))
            while len(last_name.strip()) == 0:
                last_name = str(input(prompts["empty_line_error"]))
            if last_name == "0":
                return "1"
            lst = PeopleTable().find_by_last_name(last_name)
            if len(lst) == 0:
                print(prompts["no_rows_error"])
            else:
                for i in lst:
    ...
    def show_groups_by_people(self):
        if self.person_id == -1:
            while True:
                num = input(prompts["empty_line"])
                while len(num.strip()) == 0:
        ...
        print("Выбран человек: " +
              self.person_obj[1] + " " + self.person_obj[2] + " " + self.person_obj[3])
        print(prompts["show_groups"])
        lst = GroupTable().all_by_person_id(self.person_id)
        for i in lst:
            print(i[1] + "\t" + i[2] + "\t" + i[3])
        print(groups_menu)
        return self.read_next_step()
    ...
    def show_add_group(self):
        break_code = '1'
        group = self.check_input('Группа', 12, True, break_code)
        if group == break_code:
            return break_code
        speciality = self.check_input('Специальность', 12, True, break_code)
        if speciality == break_code:
            return break_code
        kaf = self.check_input('Кафедра', 12, True, break_code)
        if kaf == break_code:
            return break_code
        exist = GroupTable().check_field_exist(
            ['person_id', 'class'], [self.person_id, group])
        print(groups_menu)
        if exist:
            print(prompts["primary_forbidden"])
            return self.read_next_step()
        GroupTable().insert_one([self.person_id, group, speciality, kaf])
        return self.read_next_step()

    def delete_group(self):
        return self.delete_by_field(GroupTable, groups_menu, 'class')
